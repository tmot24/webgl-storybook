...
interface CreateFramebuffer {
  ...
  isDepth?: boolean; // нужен ли буфер глубины (для 3D-сцены внутри)
}
 ...
 * */
export function createFramebuffer({ gl, width, height, destroyRef, isDepth = true }: CreateFramebuffer) {
  /**
   * 1. Создание объекта буфера кадра
   * */
  ...
  /**
   * 2. Создание объекта текстуры и задание его размеров и параметры
   * */
  ...
  /**
   * 3. Создание depth-renderbuffer - глубина для 3D внутри framebuffer
  ...
  if (isDepth) {
    ...
    /**
     * 4. Связать объект depthBuffer с его типом и задать размер
     * */
    ...
    /**
     * 5. Подключение объекта depthBuffer к framebuffer
     * */
    /**
  ...
  /**
   * 5. Подключение объекта texture к framebuffer
   * */
  /**
