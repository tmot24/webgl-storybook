...
export class ZFighting {
  ...
  private readonly points = [
    ...
    { coord: { x: 0.0, y: 2.5, z: 0.0 }, color: { r: 0.4, g: 1.0, b: 0.4 } }, // верх
    { coord: { x: -2.5, y: -2.5, z: 0.0 }, color: { r: 0.4, g: 1.0, b: 0.4 } }, // лево
    { coord: { x: 2.5, y: -2.5, z: 0.0 }, color: { r: 1.0, g: 0.4, b: 0.4 } }, // право
    // Жёлтый треугольник
    { coord: { x: 0.0, y: 3.0, z: 0.0 }, color: { r: 1.0, g: 1.0, b: 0.4 } }, // верх
    { coord: { x: -3.0, y: -3.0, z: 0.0 }, color: { r: 1.0, g: 1.0, b: 0.4 } }, // лево
    { coord: { x: 3.0, y: -3.0, z: 0.0 }, color: { r: 1.0, g: 0.4, b: 0.4 } }, // право
  ];

  constructor() {
    const { viewMatrix } = injectOrbitCamera({
      canvasRef: this.canvas,
      initialEye: vec3.fromValues(0, 0, 15),
    });
    ...
    injectWebGLRender({
      ...
      setup: ({ gl, program, destroyRef }) => {
        ...
        // Активирует функцию добавления сдвига к глубине
      ...
      render: ({ gl, width, height, setup: { count, vao, u_Matrix } }) => {
        ...
        gl.polygonOffset(0.0, 0.0); // явно: без смещения
         ...
         * где m представляет наклон треугольника относительно линии взгляда,
         * а r - наименьшая разница между двумя значениями координаты Z, которые могут оказаться неразличимыми для WebGL
         * */
        gl.polygonOffset(1.0, 1.0); // со смещением
