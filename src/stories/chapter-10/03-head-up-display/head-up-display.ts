...
export class HeadUpDisplay {
  ...
  private readonly hub = viewChild.required<ElementRef<HTMLCanvasElement>>('hubRef');
  ...
  constructor() {
    ...
    injectWebGLRender({
      ...
      setup: ({ gl, program, destroyRef }) => {
         ...
         * HUB
         * */
        const ctx = this.hub().nativeElement.getContext('2d');
        if (!ctx) throw new Error('WebGL2 не поддерживается');

        this.hub().nativeElement.width = 400;
        this.hub().nativeElement.height = 400;
        ...
        ctx.fillText('HUB: Head Up Display', 40, 180);
